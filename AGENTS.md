...
- Los cálculos de saldo y agenda viven en `admin/finanzas.js`, con pruebas en `tests/admin-finanzas.test.js`. Respetar saldo explícito $0; una inscripción sin precio queda sin valorar. Una campaña general o una fecha calculada no es un acuerdo. Priorizar cuotas pendientes estructuradas y acuerdos posteriores; repartir el saldo restante sin duplicarlo y marcarlo sin fecha cuando corresponda. Esperar antecedentes o revisar identidad nunca implica una promesa de pago. La caja histórica conserva los abonos de bajas, separados de la proyección activa.
