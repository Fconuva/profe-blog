...
const DIL_RUTS_PATH = path.join(ROOT, 'lecturas', 'asignaciones', 'pedro_paramo_nm3_3A_tp_dil_ruts.txt');
const LEVE_RUTS_PATH = path.join(ROOT, 'lecturas', 'asignaciones', 'pedro_paramo_nm3_3A_tp_leve_ruts.txt');
const SESSION_CANDIDATES = [
  ...
  { sid: 'pedro-paramo-nm3-darwin-torres-pie-2026', source: 'pie' },
  { sid: 'pedro-paramo-nm3-mahikol-quinteros-pie-2026', source: 'pie' },
  { sid: 'pedro-paramo-nm3-benjamin-verdugo-pie-2026', source: 'pie' }
];

const DIL_RUTS = new Set(
  fs.readFileSync(DIL_RUTS_PATH, 'utf8')
    .split(/\r?\n/)
    .map(line => cleanRut(line))
    .filter(Boolean)
);

const LEVE_RUTS = new Set(
  fs.readFileSync(LEVE_RUTS_PATH, 'utf8')
    .split(/\r?\n/)
    .map(line => cleanRut(line))
    .filter(Boolean)
);
...
(async () => {
  try {
    ...
    const rows = roster.map(entry => {
      const normalizedRut = cleanRut(entry.rut);
      const expectedSource = DIL_RUTS.has(normalizedRut) ? 'dil' : (LEVE_RUTS.has(normalizedRut) ? 'leve' : 'regular');
