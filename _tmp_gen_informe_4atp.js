// Genera informe Maus NM4 4A-TP (clon 4D-TP, sin exclusiones por defecto)
...
const CURSO = '4A-TP';
const ROSTER_FILE = path.join(ROOT, 'carga_4A_TP.txt');
const REPORT_DIR = path.join(ROOT, 'lecturas', 'adminprofe', 'reportes');
if (!fs.existsSync(REPORT_DIR)) fs.mkdirSync(REPORT_DIR, { recursive: true });
const HTML_OUT = path.join(REPORT_DIR, 'Informe_Final_Maus_4A_TP_2026.html');
const CSV_OUT = path.join(REPORT_DIR, 'Planilla_Maus_4A_TP_2026.csv');
const LIRMI_OUT = path.join(REPORT_DIR, 'Plantilla_Lirmi_Maus_4A_TP_2026.csv');
const EXIGENCIA = 0.6;
