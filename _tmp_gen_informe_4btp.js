// Genera informe de resultados Maus NM4 4B-TP 2026 imitando estilo 3D-TP
// Sin respuestas reales => AUSENTE
...
const ROSTER_FILE = path.join(ROOT, 'carga_4B_TP.txt');
const REPORT_DIR = path.join(ROOT, 'lecturas', 'adminprofe', 'reportes');
if (!fs.existsSync(REPORT_DIR)) fs.mkdirSync(REPORT_DIR, { recursive: true });

const HTML_OUT = path.join(REPORT_DIR, 'Informe_Final_Maus_4B_TP_2026.html');
const CSV_OUT = path.join(REPORT_DIR, 'Planilla_Maus_4B_TP_2026.csv');
const LIRMI_OUT = path.join(REPORT_DIR, 'Plantilla_Lirmi_Maus_4B_TP_2026.csv');
...
const SESSIONS_MAUS = [
  ...
  { sid: 'maus-nm4-jose-tomas-calderon-pie-2026', file: 'pie/maus-nm4-jose-tomas-calderon-pie-2026.json' },
];
...
// Carga sesiones (items + auto totales)
function loadSession(file) {
  const fp = path.join(ROOT, 'lecturas', 'contenidos', file);
  if (!fs.existsSync(fp)) return null;
  const data = JSON.parse(fs.readFileSync(fp, 'utf8'));
  // collect items recursively from materiales[].items + top-level items + actividades[].items
  const items = [];
  function walk(arr) {
    if (!Array.isArray(arr)) return;
    arr.forEach(o => {
      if (o && o.id && o.tipo && (o.enunciado || o.pregunta || o.texto)) {
        items.push(o);
      }
  ...
  walk(data.items || []);
  walk(data.preguntas || []);
  walk(data.materiales || []);
  walk(data.actividades || []);
  walk(data.secciones || []);
  // dedup by id
  const seen = new Set();
  ...
  return { items: itemsMeta, autoTotal, totalAll, manualCount, manualTotal: totalAll - autoTotal, raw: data };
}

// Evalua una respuesta contra la clave correcta (usa misma logica que prueba.html)
function evaluarItem(item, answer) {
  ...
  return null; // desarrollo / respuesta_corta
}
...
(async () => {
  ...
  const [studentsSnap, asigSnap, respSnap, resSnap, sesSnap] = await Promise.all([
  ...
  for (const ss of SESSIONS_MAUS) {
    ...
    if (sd) {
      sessionsData[ss.sid] = sd;
      console.log('[session]', ss.sid, '· items=' + sd.items.length, '· auto=' + sd.autoTotal, '· total=' + sd.totalAll);
    } else {
      console.warn('[session] NOT FOUND:', ss.file);
    }
  }
  ...
  for (const r of roster) {
    ...
    if (!stu) {
      filas.push({ ...base, estado: 'AUSENTE', motivo: 'No registrado en plataforma' });
      continue;
    }
    const uid = stu.uid;

    // Buscar la sesion donde el estudiante tiene resultados o respuestas
    let sidUsada = null;
    let resUsado = null;
    let respUsado = null;
    let mejorScore = -1;
    ...
    if (!sidUsada || (!resUsado && !respUsado)) {
      filas.push({ ...base, estado: 'AUSENTE', motivo: 'Sin actividad' });
      continue;
    }
    ...
    const submitted = !!(resUsado || (respUsado && respUsado.submitted_at) || (respUsado && respUsado.completada));
    ...
    if (finalAutoMax === 0) finalAutoMax = (sd && sd.autoTotal) || 25;

    // Nota se calcula sobre auto (descartando manual pendiente — politica conservadora)
    ...
    filas.push({
      ...
      pct,
      nota,
      _detalle: (resUsado && resUsado.detalle) || null,
      _answers: (respUsado && respUsado.answers) || null,
      _sd: sd,
    });
  }

  // Orden alfabetico para tabla
  filas.sort((a, b) => a.nombre.localeCompare(b.nombre));

  // Estadisticas (solo evaluados)
  ...
  filas.forEach((f, i) => {
    csv += [
      ...
      csvEscape(f.sid || ''),
      csvEscape(f.estado),
      f.autoPts != null ? f.autoPts : '',
      f.autoMax != null ? f.autoMax : '',
      f.pct != null ? f.pct + '%' : '',
      f.nota != null ? f.nota.toFixed(1).replace('.', ',') : '',
      f.manualPend != null ? f.manualPend : '',
      f.answeredCount != null ? f.answeredCount : '',
      f.totalItems != null ? f.totalItems : '',
      csvEscape(f.motivo || ''),
    ].join(';') + '\n';
  });
  ...
  // CSV Lirmi (solo Nombre;RUT;Nota — pendientes/ausentes con celda vacia)
  ...
  function rowHtml(f, idx) {
    if (f.estado === 'PENDIENTE') {
      return `<tr style="background:#fef3c7">
        <td class="name-cell">${htmlEscape(f.nombre)}</td>
        <td style="text-align:center;font-size:10px;color:#666">${htmlEscape(f.rut)}</td>
        <td colspan="6" style="text-align:center;color:#92400e;font-weight:700">⏳ Pendiente — se le dará más tiempo</td>
        <td class="mc-cell">—</td>
        <td style="text-align:center"><span style="background:#fde68a;color:#92400e;padding:3px 8px;border-radius:6px;font-size:9px;font-weight:700">PENDIENTE</span></td>
      </tr>`;
    }
    if (f.estado === 'AUSENTE') {
      return `<tr style="background:#fee2e2">
        <td class="name-cell">${htmlEscape(f.nombre)}</td>
        <td style="text-align:center;font-size:10px;color:#666">${htmlEscape(f.rut)}</td>
        <td colspan="6" style="text-align:center;color:#991b1b;font-weight:600">${htmlEscape(f.motivo || 'Ausente')}</td>
        <td class="mc-cell">—</td>
        <td style="text-align:center"><span style="background:#fecaca;color:#991b1b;padding:3px 8px;border-radius:6px;font-size:9px;font-weight:700">AUSENTE</span></td>
      </tr>`;
    }
    ...
    return `<tr>
      <td class="name-cell" title="${htmlEscape(f.nombre)}">${htmlEscape(f.nombre)}</td>
      <td style="text-align:center;font-size:10px;color:#666">${htmlEscape(f.rut)}</td>
      <td class="mc-cell">${f.autoPts}/${f.autoMax}</td>
      <td class="mc-cell">${f.answeredCount}/${f.totalItems}</td>
      <td class="mc-cell">${f.manualPend || 0}</td>
      <td class="mc-cell">${f.totalPrueba || (f.autoMax + (f.manualPts || 0))}</td>
      <td class="mc-cell">—</td>
      <td class="mc-cell" style="font-weight:700">${f.autoPts}/${f.autoMax}</td>
      <td class="mc-cell">${f.pct}%</td>
      <td class="mc-cell" style="font-size:14px;font-weight:800;color:${colorNota}">${f.nota.toFixed(1)}</td>
      <td style="text-align:center"><span style="background:${bgEstado};color:${colorEstado};padding:3px 8px;border-radius:6px;font-size:9px;font-weight:700">${f.estado}</span></td>
    </tr>`;
  }

  const tablaHtml = filas.map(rowHtml).join('\n');

  // === DETALLE POR PREGUNTA POR ESTUDIANTE ===
  function buildPerItemBlocks(f) {
    ...
    const chips = sd.items.map(it => {
      let status; // ok | bad | empty | manual
      let respuesta = '';
      const det = detalle[it.id];
      if (det) {
        if (det.autoEvaluable === false || (it.tipo === 'respuesta_corta' || it.tipo === 'desarrollo' || it.tipo === 'short_text')) status = 'manual';
        ...
        if (it.tipo === 'respuesta_corta' || it.tipo === 'desarrollo' || it.tipo === 'short_text') status = 'manual';
        else if (a == null || a === '' || (typeof a === 'object' && Object.keys(a).length === 0)) status = 'empty';
        else {
          const ev = evaluarItem(it, a);
          if (ev === true) status = 'ok';
          else if (ev === false) status = 'bad';
          else status = 'manual';
        }
        respuesta = a != null ? a : '';
      }
      ...
      return `<span class="qchip" style="background:${colors.bg};color:${colors.fg}" title="${htmlEscape(tip)}"><b>P${it.indice}</b> ${colors.icon}</span>`;
    }).join(' ');
    let oks=0, bads=0, emptys=0, mans=0;
    sd.items.forEach(it => {
      const det = detalle[it.id]; const a = answers[it.id];
      if (it.tipo === 'respuesta_corta' || it.tipo === 'desarrollo' || it.tipo === 'short_text') { mans++; return; }
      if (det) { if (det.esCorrecta === true) oks++; else if (det.esCorrecta === false) bads++; else emptys++; }
      else if (a == null || a === '') emptys++;
      else { const ev = evaluarItem(it, a); if (ev===true) oks++; else if (ev===false) bads++; else mans++; }
    });
    return `<div class="student-detail">
      <div class="sd-head">
        <strong>${htmlEscape(f.nombre)}</strong>
        <span class="sd-meta">${htmlEscape(f.rut)} · Nota <b style="color:${f.nota>=6?'#16a34a':f.nota>=4?'#1e3a5f':'#dc2626'}">${f.nota.toFixed(1)}</b> · ${f.pct}%</span>
        <span class="sd-tally"><span style="color:#166534">✓ ${oks}</span> · <span style="color:#991b1b">✗ ${bads}</span>${emptys?` · <span style="color:#64748b">— ${emptys}</span>`:''}${mans?` · <span style="color:#5b21b6">✏️ ${mans}</span>`:''}</span>
      </div>
...
<title>Informe Final — Maus NM4 — 4°B T.P. — 2026</title>
<style>
...
.header-left h1 { font-size: 22px; color: #0369a1; line-height: 1.2; }
...
.curso-badge { background: linear-gradient(135deg,#0369a1,#0ea5e9); color: #fff; padding: 10px 24px; border-radius: 10px; font-size: 20px; font-weight: 800; text-align: center; letter-spacing: 0.5px; }
...
.card-big { font-size: 28px; font-weight: 800; color: #0369a1; line-height: 1; }
...
th { background: #0369a1; color: #fff; padding: 6px 8px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.3px; }
...
.print-bar button { padding: 10px 32px; background: linear-gradient(135deg,#0369a1,#0ea5e9); color: #fff; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 600; }
.notas-bar { display:flex; height:30px; border-radius:8px; overflow:hidden; margin: 10px 0; font-size:11px; font-weight:700; }
.notas-ap { background:#16a34a; color:#fff; display:flex; align-items:center; justify-content:center; }
.notas-rep { background:#dc2626; color:#fff; display:flex; align-items:center; justify-content:center; }
.notas-pend { background:#eab308; color:#422006; display:flex; align-items:center; justify-content:center; }
.notas-aus { background:#94a3b8; color:#fff; display:flex; align-items:center; justify-content:center; }
...
<div class="page">
  <div class="header">
    <div class="header-left">
      <h1>Informe de Resultados — Prueba Maus (NM4)</h1>
      <h2>Lengua y Literatura NM4 — 4°B T.P. — Año 2026</h2>
      <p class="sub">Centro Educativo Salesianos Talca | Prof. Francisco Javier Núñez Valenzuela | Generado: ${fechaGen}</p>
    </div>
    <div>
      <div class="curso-badge">4°B T.P.</div>
  ...
  <div class="cards">
    <div class="card card-highlight">
      <div class="card-big">${filas.length}</div>
      <div class="card-label">Total Curso</div>
      <div class="card-sub">${N} evaluados · ${pendientes} pendientes · ${ausentes} ausentes</div>
    </div>
    <div class="card">
      <div class="card-big">${promNota}</div>
      <div class="card-label">Promedio Curso</div>
    ...
    <div class="card">
      <div class="card-big">${aprob}/${N}</div>
      <div class="card-label">Aprobados (≥ 4,0)</div>
      <div class="card-sub">${repr} reprobados</div>
    </div>
  </div>

  <div class="notas-bar">
    ${aprob ? `<div class="notas-ap" style="flex:${aprob}">✅ ${aprob} aprobados</div>` : ''}
    ${repr ? `<div class="notas-rep" style="flex:${repr}">❌ ${repr} reprobados</div>` : ''}
    ${pendientes ? `<div class="notas-pend" style="flex:${pendientes}">⏳ ${pendientes} pendientes</div>` : ''}
    ${ausentes ? `<div class="notas-aus" style="flex:${ausentes}">— ${ausentes} ausentes</div>` : ''}
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th>Auto</th>
        <th>Resp.</th>
        <th>Manual<br>Pend.</th>
        <th>Total<br>Prueba</th>
        <th>Desarrollo</th>
        <th>Subtotal</th>
        <th>%</th>
        <th>Nota</th>
  ...
  <div style="font-size:11px;color:#475569;line-height:1.6">
    <p><strong>Pendientes:</strong> Bastián Valenzuela (22.816.174-8) y Amaro Verdejo (22.472.709-7) quedan pendientes por temas de tiempo. Se les dará una nueva oportunidad.</p>
    <p><strong>Ausentes:</strong> estudiantes sin registro de actividad en la plataforma.</p>
    <p><strong>Cálculo de nota:</strong> exigencia 60% sobre puntaje auto-evaluable (selección múltiple, V/F, pareados, ordenamiento). Las preguntas de desarrollo quedan pendientes de revisión manual y se incorporarán en una segunda nota.</p>
  </div>
  ...
  <p style="font-size:11px;color:#64748b;margin-bottom:10px">
    <span class="qchip" style="background:#dcfce7;color:#166534">P# ✓</span> correcta ·
    <span class="qchip" style="background:#fee2e2;color:#991b1b">P# ✗</span> incorrecta ·
    <span class="qchip" style="background:#f1f5f9;color:#64748b">P# —</span> sin responder ·
    <span class="qchip" style="background:#ede9fe;color:#5b21b6">P# ✏️</span> desarrollo (revisión manual).
    Pasa el cursor sobre cada chip para ver el tipo de pregunta y el estado.
  </p>
  ${detallesHtml}

  <div class="footer">
    Informe generado automáticamente · ${fechaGen} · Plataforma Lecturas profefranciscopancho.com
  </div>
</div>
</body>
</html>`;
