...
if (!admin.apps.length) admin.initializeApp({ credential: admin.credential.cert({ projectId: process.env.FIREBASE_PROJECT_ID, clientEmail: process.env.FIREBASE_CLIENT_EMAIL, privateKey: normalizePrivateKey(process.env.FIREBASE_PRIVATE_KEY) }), databaseURL: process.env.FIREBASE_DATABASE_URL || 'https://profe-blog-default-rtdb.firebaseio.com' });
const db = admin.database();
const ROOT = __dirname;
const CURSO = '4C-TP';
const ROSTER_FILE = path.join(ROOT, 'carga_4C_TP.txt');
const REPORT_DIR = path.join(ROOT, 'lecturas', 'adminprofe', 'reportes');
if (!fs.existsSync(REPORT_DIR)) fs.mkdirSync(REPORT_DIR, { recursive: true });
const HTML_OUT = path.join(REPORT_DIR, 'Informe_Final_Maus_4C_TP_2026.html');
const CSV_OUT = path.join(REPORT_DIR, 'Planilla_Maus_4C_TP_2026.csv');
const LIRMI_OUT = path.join(REPORT_DIR, 'Plantilla_Lirmi_Maus_4C_TP_2026.csv');
const EXIGENCIA = 0.6;
const PENDIENTES_RUT = new Set();
const AJUSTES_NOTA = new Map([
  ['22952227-2', 0.3],
]);
...
(async () => {
  ...
  for (const r of roster) {
    ...
    const ajusteNota = AJUSTES_NOTA.get(r.rut) || 0;
    const notaBase = calcularNota(finalAutoPts, finalAutoMax, EXIGENCIA);
    const nota = Math.min(7.0, +(notaBase + ajusteNota).toFixed(1));
