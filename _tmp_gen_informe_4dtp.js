// Genera informe Maus NM4 4D-TP (clon 4B-TP, sin exclusiones por defecto)
...
const CURSO = '4D-TP';
const ROSTER_FILE = path.join(ROOT, 'carga_4D_TP.txt');
const REPORT_DIR = path.join(ROOT, 'lecturas', 'adminprofe', 'reportes');
if (!fs.existsSync(REPORT_DIR)) fs.mkdirSync(REPORT_DIR, { recursive: true });
const HTML_OUT = path.join(REPORT_DIR, 'Informe_Final_Maus_4D_TP_2026.html');
const CSV_OUT = path.join(REPORT_DIR, 'Planilla_Maus_4D_TP_2026.csv');
const LIRMI_OUT = path.join(REPORT_DIR, 'Plantilla_Lirmi_Maus_4D_TP_2026.csv');
const EXIGENCIA = 0.6;
...
const SESSIONS_MAUS = [
  ...
  { sid: 'maus-nm4-exequiel-ramirez-pie-2026', file: 'pie/maus-nm4-exequiel-ramirez-pie-2026.json' },
];
