(function () {
  ...
  const kinds = ['Compañero 1', 'Compañero 2', 'Compañero 3', 'Comunidad educativa 1', 'Comunidad educativa 2'];
  ...
  function updateProgress() {
    ...
    const baseWritten=bookGuide.filter(s=>!s.optional&&state.bookSections[s.id]?.trim()).length;const optionsWritten=bookGuide.filter(s=>s.optional&&state.bookSections[s.id]?.trim()).length;
    $('bookStatus').textContent=baseWritten+' partes del libro con texto · '+optionsWritten+' capítulos a elección iniciados. Puedes combinar secciones; no debes completar todas las opciones.';
